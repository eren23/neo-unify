"""Exp CFG: Generation with RK2 solver, CFG inference, and guidance scale sweep (PyTorch)."""
...
def load_model(device=None, use_ema=True):
    ...
    weight_file = f"{WEIGHTS_DIR}/exp_cfg_ema_torch.pt" if use_ema else f"{WEIGHTS_DIR}/exp_cfg_torch.pt"
...
def generate(samples_per_class=8, num_steps=100):
    print("=" * 60)
    print("Exp CFG: Generation with RK2 + CFG [PyTorch]")
    print("=" * 60)
    ...
    for gs in guidance_scales:
        ...
        save_image_grid(
            all_images_cat, f"exp_cfg_gs{gs:.1f}_torch.png",
    ...
    save_image_grid(
        recon_grid, "exp_cfg_recon_torch.png",
