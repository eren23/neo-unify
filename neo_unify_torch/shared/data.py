"""Synthetic 16x16 RGB dataset generator for unified multimodal experiments (PyTorch)."""
...
def generate_dataset(samples_per_class=1000, seed=42):
    ...
    Returns:
        images: torch.Tensor of shape (N, 3, 16, 16) in [0, 1] (NCHW)
        labels: torch.LongTensor of shape (N,) with class indices
    """
    ...
    # NHWC -> NCHW
    images = torch.from_numpy(np.stack(all_images)).float().permute(0, 3, 1, 2)
